...
defmodule OMG.Watcher.Web.Controller.UtxoTest do
  ...
  # FIXME should 500 be returned here?
  @tag fixtures: [:initial_blocks]
  test "getting exit data returns error when there is no txs in specfic block" do
    ...
    assert %{
             ...
             "result" => "error"
           } = TestHelper.rest_call(:post, "/utxo.get_exit_data", %{"utxo_pos" => utxo_pos}, 500)
  end

  @tag fixtures: [:initial_blocks]
  test "getting exit data endpoint returns error when there is no tx in specfic block" do
